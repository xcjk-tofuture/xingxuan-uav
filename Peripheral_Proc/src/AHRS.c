...
void AHRS_Uart4_IDLE_Proc(u8 size)
{
	...
	{
		#if EXTERN_IMU
		ahrs_data.rollSpeed=DATA_Trans(uart4RX[7],uart4RX[8],uart4RX[9],uart4RX[10]) * DEG_PER_RAD;       //横滚角速度
		ahrs_data.pitchSpeed=DATA_Trans(uart4RX[11],uart4RX[12],uart4RX[13],uart4RX[14]) * DEG_PER_RAD;   //俯仰角速度
		ahrs_data.yawSpeed=DATA_Trans(uart4RX[15],uart4RX[16],uart4RX[17],uart4RX[18]) * DEG_PER_RAD; //偏航角速度
			
    ahrs_data.roll=DATA_Trans(uart4RX[19],uart4RX[20],uart4RX[21],uart4RX[22]) * DEG_PER_RAD;      //横滚角
		ahrs_data.pitch=DATA_Trans(uart4RX[23],uart4RX[24],uart4RX[25],uart4RX[26]) * DEG_PER_RAD;     //俯仰角
		ahrs_data.yaw=DATA_Trans(uart4RX[27],uart4RX[28],uart4RX[29],uart4RX[30]) * DEG_PER_RAD;	 //偏航角
			
		ahrs_data.q0=DATA_Trans(uart4RX[31],uart4RX[32],uart4RX[33],uart4RX[34]);  //四元数
...
{
	if(GyroCalFlag)
		Simple_Zero_Offset_Calibration(imu, &(imu->gyrobias));  //简单零偏误差校准
	if(AccCalFlag)
